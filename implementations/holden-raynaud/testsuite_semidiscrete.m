...
%% Solve equation
[U, x, tu] = holdenraynaud(N, T, [xmin, xmax], initial, 'showprogress', true);
[V, x, tv] = holdenraynaud_semidiscrete(N, T, [xmin, xmax], initial, 'showprogress', true);
% M = size(U, 1);
...
% nx = min(nx, N);
% nt = min(nt, M);
[Z, xcomp, tcomp] = compress(x, tu, U, nx, nt);
[Y, ~, ~] = compress(x, tv, V, nx, nt);

%% Plotting
figure
surf(xcomp, tcomp, Z)
shading flat;
animatedplot(xcomp, tcomp, Z, Y)
xlabel('x')
ylabel('y')